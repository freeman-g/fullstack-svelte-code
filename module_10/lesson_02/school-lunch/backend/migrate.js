...
const migrateDatabases = async () => {
  ...
  const catalogDb = await defaultKnex.select().from('pg_database').where({ datname: 'catalog' }).first()
  ...
  for (let i = 0; i < schools.length; i++) {
    ...
    const db = await catalogKnex.select().from('pg_database').where({ datname: school.school_database }).first()
